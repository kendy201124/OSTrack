...
class VisionTransformerMFFD(VisionTransformer):
    ...
    def forward_features(self, z, x):
        ...
        for i, blk in enumerate(self.blocks):
            ...
            if i in self.mff_loc:
                ...
                z_recover = x[:,:lens_z]
                x_recover = x[:,lens_z:] #(1,256,768)
                x_recover = recover_tokens(x_recover, lens_z, lens_x, mode=self.cat_mode)
                if i != self.mff_loc[-1]:
        ...
        x = combine_tokens(z_recover, res, mode=self.cat_mode)
        #lens_z = self.pos_embed_z.shape[1]
        #lens_x = self.pos_embed_x.shape[1]
...
def _create_vision_transformer(pretrained=False, **kwargs):
    model = VisionTransformerMFFD(**kwargs)
...
def vit_base_patch16_224_mffd(pretrained=False, **kwargs):
...
def vit_large_patch16_224_mffd(pretrained=False, **kwargs):
